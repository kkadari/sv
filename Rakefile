...
namespace :features do
  ...
  Cucumber::Rake::Task.new(:non_ui) do |t|
    t.profile = 'standard'
    t.cucumber_opts = 'browser=firefox -t ~@ui -t ~@sit -t ~@manual'
  end

  Cucumber::Rake::Task.new(:cert_ce_sv_ref_firefox) do |t|
    t.profile = 'standard'
    t.cucumber_opts = 'browser=firefox -t ~@sit -t ~@manual'
  end

  Cucumber::Rake::Task.new(:cert_ce_sv_ref_chrome) do |t|
    t.profile = 'standard'
    t.cucumber_opts = 'browser=chrome -t ~@sit -t ~@manual'
  end

  Cucumber::Rake::Task.new(:cert_ce_sv_ref_phantom) do |t|
    t.profile = 'standard'
    t.cucumber_opts = 'browser=phantom -t ~@sit -t ~@manual'
  end
  ...
  Cucumber::Rake::Task.new(:wip) do |t|
    t.profile = 'standard'
    t.cucumber_opts = 'browser=firefox -t @wip -t ~@manual'
  end
end
