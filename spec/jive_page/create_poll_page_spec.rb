...
describe 'Create poll page' do
  ...
  it 'should return a 200 when requesting to move a poll choice' do
    RestClient.get('http://dev188.sure.vine/poll/create.jspa?sr=cmenu&containerType=2020&containerID=1024',:cookie => @authorisation){|response|
      @poll_id = response.body.scan(/name="pollID" value="([0-9]*)/).join(',')
    }

    RestClient.post(ENV['base_url'] + '/__services/v2/rest/polls/' + @poll_id + '/pollOptions','',:cookie => @authorisation){|response|
      @choice_id = (JSON.parse(response)['id'] - 1).to_s
    }

    RestClient.post(ENV['base_url'] + '/__services/v2/rest/polls/' + @poll_id + '/pollOptions/' + @choice_id.to_s + '/index','index=0',{:cookie => @authorisation,:content_type => 'application/x-www-form-urlencoded'}){|response|
      fail('Failed with ' + response.code.to_s) if response.code != 200
    }
  end
