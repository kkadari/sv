...
describe 'The profile activity page' do
  ...
  it 'should return a 200 when requesting similar users' do
    RestClient.get(ENV['base_url'] + '/similar-users.jspa?targetUser=2012',:cookie => @authorisation){|response|
      fail('Failed with ' + response.code.to_s) if response.code != 200
    }
  end

  it 'should return a 200 when requesting most liked' do
    RestClient.get(ENV['base_url'] + '/most-liked.jspa?containerType=14&container=1&numResults=10&filterUserID=2012',:cookie => @authorisation){|response|
      fail('Failed with ' + response.code.to_s) if response.code != 200
    }
  end
