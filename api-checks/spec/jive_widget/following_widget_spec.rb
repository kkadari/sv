...
describe 'Following widget' do
  ...
  # Uncommented as following appears to have changed in Jive 8
  # it 'should return a 200 when requesting to follow a user in a specific feed' do
  #  RestClient.post(ENV['base_url'] + '/__services/v2/rest/stream-config/11/objects','[{"objectType":3,"objectID":' + @user_2_id + '}]',
  #                  {:cookie => @authorisation,:content_type => 'application/json'}){|response|
  #    assert_code_and_body(response, 200)
  #  }
  # end
  ...
  #  RestClient.post(ENV['base_url'] + '/__services/v2/rest/stream-config/11/objects/remove','[{"objectType":3,"objectID":' + @user_2_id + '}]',
  #                  {:cookie => @authorisation,:content_type => 'application/json'}){|response|
  #    assert_code_and_body(response, 200)
  #  }
  # end
