...
module Request
  ...
  #                        All    = Hash['type','all']
  #                        Person = Hash['type','person','users','user1@test,user2@test'] - Note that each user you want is comma separated
  #                        Place  = Hash['type','place','group','testing'] or Hash['type','place','group','secret-test-group']
  ...
  def set_publish_bar(publication)
    case publication[:type]
      ...
      when 'place'
        case publication[:group]
          when 'testing'
            return '{"visibility":"place","container":{"objectType":"700","objectID":"1004"}}'
          when 'secret-test-group'
            return '{"visibility":"place","container":{"objectType":"700","objectID":"1005"}}'
          else
            fail "Allowed publication groups are testing or secret-test-group.  You chose: #{publication[:group]}"
        end
      else
        fail "Allowed publications are all, person and place.  You chose: #{publication[:type]}"
    end
  end

  def anonymous(anonymous_flag)
    if anonymous_flag
      return "true"
    else
      return "false"
