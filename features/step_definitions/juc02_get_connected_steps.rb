Given /^that "([^"]*)" and "([^"]*)" are not following one another$/ do |user1, user2|
  ...
  Follow.delete_follow_connection_v3(user_1_profile[:user_id], user_2_profile[:user_id], @browser.cookies.to_a)
end
...
When /^"([^"]*)" accepts the connection request$/ do |user|
  ...
  response = Inbox.get_connections_inbox_message(@browser.cookies.to_a)
  ...
  Follow.post_action_follow_request(payload, @browser.cookies.to_a)
end
...
Then /^"([^"]*)" is not listed as a follower of "([^"]*)"$/ do |user1, user2|
  ...
  response = Profile.get_people_followed(user_1_profile[:username], @browser.cookies.to_a)
...
When /^I sever the connection with "([^"]*)" as "([^"]*)"$/ do |user1, user2|
  ...
  Follow.delete_follow_connection(user_profile[:user_id], @browser.cookies.to_a)
end