Feature: [CISP-608] Participant views another Profile which contains IR posted Anonymously
...
#</ul>
  ...
  Scenario: Participant views another Profile which contains IR posted Anonymously
	Given I log in
	...
	Then I presented with a page for creating a new IR
    And the page is pre-populated with a default IHM level: Amber
	...
	And I discover a user of interest and click a link to their Profile
	  | {User A's profile} |
	Then the system will check that the Profile exists and is valid, and if so, displays the Profile. However, content created Anonymously by the Profile owner {by User A in Step 6} is NOT shown.
